...
{
    public partial class service_sale_form : Form
    {
        int indexRow = 0, id_trade;
        SqlDataAdapter sda = new SqlDataAdapter();
        public service_sale_form()
        {
            InitializeComponent();
        }

        private void service_sale_form_Load(object sender, EventArgs e)
        {
        ...
        {
            Add_edit_position_service aeps = new Add_edit_position_service();
            aeps.ShowDialog();
            if (aeps.DialogResult == DialogResult.OK)
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                Add_edit_position_service aeps = new Add_edit_position_service((int)dataGridView1.Rows[indexRow].Cells[0].Value, dataGridView1.Rows[indexRow].Cells[1].Value.ToString(),
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                //вставка в таблицу trade_service
                //цикл для каждой строки
                SqlDataAdapter sda2 = new SqlDataAdapter();
                for (int i = 0; i < dataGridView1.Rows.Count; i++)
                {
                    sda2.InsertCommand = new SqlCommand("INSERT INTO trade_service (id_trade, id_service, quantity) VALUES (@id_trade, @id_service, @quantity)", sqlconnect.sql_con);
                    sda2.InsertCommand.Parameters.Add(new SqlParameter("@id_trade", SqlDbType.Int));
                    sda2.InsertCommand.Parameters.Add(new SqlParameter("@id_service", SqlDbType.Int));
                    ...
                    sda2.InsertCommand.Parameters["@id_service"].Value = (int)dataGridView1.Rows[i].Cells[0].Value;
